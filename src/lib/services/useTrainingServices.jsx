...
import { fetchTrainingFail, fetchTrainingStart, fetchTrainingSuccess, fetchTrainingSuccessWithOutPayload } from "../features/trainingSlice";
...
const useTrainingServices = () => {
    ...
    const deleteTrainingMember = async(id) => {
        ...
        try {
          ...
          dispatch(fetchTrainingFail());
          toastErrorNotify(error?.response?.data?.message || "Deleting Training member is failed!");
          console.log("delete training member api error:", error);
        }

    }
  return {getTrainingApi, createNewTrainingMember, updateTrainingMember, deleteTrainingMember}
}
